import cv2
import json
from AlgorithmServer.utils import canny_ocr
import numpy as np
from django.http import HttpResponse

from AlgorithmServer.utils.canny_ocr import draw_rectangle_show_save, print_group
...
def layout_cal(bounding, resolution):
    """
...
def exec(record, path, coordinate):
    img = cv2.imread(path)
    groups = layout_cal(process_bounding(img, canny_ocr.extract(path)), (img.shape[1], img.shape[0]))
    print_group(img, groups)
    if record:
        x = int(coordinate[0])
        y = int(coordinate[1])
...
def final_cheak(record_screen_path, replay_screen_path, record_x, record_y, replay_x, replay_y):
    ...
    print("调用final_cheak方法：layout偏移debug")
    print(str(transformer_x)+","+str(transformer_y))
    print(str(replay_x)+","+str(replay_y))
    if abs(transformer_x - replay_x) < 0.1 * replay_w and abs(transformer_y - replay_y) < 0.1 * replay_h:
        return True
    return False
...
def layout(request):
    ...
    x = request.GET['x']
    y = request.GET['y']
    print(record_screen_path)
    print(replay_screen_path)
    print(x)
    print(y)
    try:
        _, res = exec(True, record_screen_path, [x, y])
        _, res2 = exec(False, replay_screen_path, res)
        if final_cheak(record_screen_path, replay_screen_path, x, y, res2[0], res2[1]):
            return HttpResponse(str(" ".join(list(map(str, res2)))))
        else:
            print("坐标点偏移过大:{},{}".format(res2[0], res2[1]))
...
if __name__ == '__main__':
    path = r"C:\Users\zpc\Desktop\MapLIRATDatabase\dianpingApp\script3\ios\step2\screenshot.png"
    img = cv2.imread(path)
    process_bounding(img, canny_ocr.extract(path))
