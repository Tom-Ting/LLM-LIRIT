...
import cv2
import numpy as np
from AlgorithmServer.utils.vgg16 import pred_all
...
def image_type_match(screen_path):
    ...
    for boundary in boundarys:
        ...
        try:
            ...
            # 判断该控件的类型
            widget_type, probability = pred_all(cropped)
            ...
            if widget_type in widget_types and widget_type != "other":
                ...
                widget_type_and_location["probability"] = probability
                # 字典列表，即json数据
    ...
    # 判断识别出的控件数是否大于5个
    if len(widget_types_and_locations) > 5:
        # 从widget_types_and_locations中选取probability最大的5个控件
        sorted_widget_types_and_locations = sorted(widget_types_and_locations, key=lambda x: x['probability'], reverse=True)
        top_5 = sorted_widget_types_and_locations[:5]

        # data_json_list：写入prompt.json的数据，初始值设为top_5
        data_json_list = top_5
    else:
        data_json_list = widget_types_and_locations

    other_data = [item for item in widget_types_and_locations if item["type"] == "other"]
    # 若存在“other”数据，则选取其中的可能性最大值
    if other_data:
        # 根据 "probability" 键对数据进行排序
        sorted_other_data = sorted(other_data, key=lambda x: x['probability'], reverse=True)

        # 选取具有最大 "probability" 值的数据
        top_1_other = sorted_other_data[0]
        data_json_list = data_json_list + top_1_other

    # 判断prompt.json文件是否已存在或为空
    if os.path.exists(prompt_path):
        ...
        if file_size != 0:
            ...
            with open(prompt_path, 'r', encoding='utf-8') as file:
                ...
                # 旧数据合并新数据
                data_json_list = olddata_json_list + data_json_list
...
def widget_type_match(widget_path):
    ...
    widget_type, probability = pred_all(widget_img)
...
if __name__ == '__main__':
    ...
    # 识别screenshot的控件类型并保存
    screen_path = r"C:\MyGraduation\database_PreExp\MapLIRATDatabase\elemaApp\script1\android\step4\screenshot.png"
